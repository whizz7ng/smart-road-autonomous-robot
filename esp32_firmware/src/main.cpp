...
#include <math.h>
#include "driver/pcnt.h"   // ESP32 하드웨어 펄스 카운터
...
void udpSend(const char* data, size_t len) {
  ...
  udpDebug.write((const uint8_t*)data, len);
  udpDebug.endPacket();
}
...
//                  엔코더 (PCNT) 설정
// =============================================================
void setupEncoderPCNT(pcnt_unit_t unit, int pin_a, int pin_b) {
  pcnt_config_t cfg = {};
...
void setupEncoders() {
  ...
  setupEncoderPCNT(PCNT_UNIT_L, ENC_L_A, ENC_L_B);
