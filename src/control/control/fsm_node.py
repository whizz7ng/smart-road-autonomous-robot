...
class FsmNode(Node):
    def __init__(self):
        ...
        # 차선 변경 (오픈 루프)
        self.declare_parameter("lane_change_rotate_az",  1.58)   # rad/s, 회전 각속도 크기 (>0)
        self.declare_parameter("lane_change_rotate_sec", 1.0)   # s, 한 번 회전 지속
        self.declare_parameter("lane_change_straight_dist_m", 0.40)  # m
        ...
        self.declare_parameter("avoid_dist_cm", 60.0)
    ...
    def tick(self):
        ...
        if self.state == STATE_STOPPED:
            ...
            if self.lc_phase == LC_PHASE_PRE_STOP:
                cmd, angular_z, speed_scale = CMD_STOP, 0.0, 0.0
            elif self.lc_phase == LC_PHASE_ROTATE_OUT:
